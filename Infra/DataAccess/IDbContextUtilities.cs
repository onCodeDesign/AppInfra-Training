...
class DbContextUtilities : IDbContextUtilities
{
    ...
    {
        Microsoft.EntityFrameworkCore.ChangeTracking.EntityEntry dbEntry = context.Entry(entity);
        return new EntityEntry(dbEntry);
    }

    public IEntityEntry<T> GetEntry<T>(T entity, DbContext context) where T : class
    {
        Microsoft.EntityFrameworkCore.ChangeTracking.EntityEntry<T> dbEntityEntry = context.Entry(entity);
