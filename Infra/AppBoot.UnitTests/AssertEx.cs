...
public static class AssertEx
{
        ...
        where TException : Exception
    {
        ...
        {
            Assert.True(assert(expected), message ?? string.Empty);
            return;
        }
        catch (Exception ex)
        {
    ...
    public static void ShouldNotThrow(this Action act)
    {
        ...
        if (exception != null)
            Assert.Fail(string.Format("Exception of type {0} was thrown when it shouldn't have been.", exception.GetType()));
    }
    ...
    public static void AreEquivalent<T>(IEnumerable<T> actual, Func<T, T, bool> equality, params T[] expected)
    {
        ...
        Assert.False(isMismatch,
            string.Format("Collections are not equivalent. Mismatch element {0}. Expected count: {1}; Actual count: {2}",
